...
func TestRouter_PolicySelection(t *testing.T) {
	...
	for _, acc := range accounts {
		account := createTestAccount(t, ts.Store, acc.id, models.ProviderOpenAI, acc.priority)
		account.InputCost = acc.inputCost
		account.OutputCost = acc.outputCost
		ts.Store.SetAccount(account)
...
func TestRouter_DistributionCalculation(t *testing.T) {
	...
	t.Run("distribution favors higher quota accounts", func(t *testing.T) {
		...
		weight1 := dist["dist-test-1"]
		weight4 := dist["dist-test-4"]
